...
public class ImFriendshipServiceImpl extends ServiceImpl<ImFriendshipMapper, ImFriendship>
  ...
  public ResponseVO updateFriend(ImFriendship imFriendship) {
    // 查询user_data表是否有用户
    ResponseVO<ImUserData> fromIdUser = imUserDataService.getUserInfoByID(imFriendship.getFromId(), imFriendship.getAppId());
    if (!fromIdUser.isOk()) {
      return fromIdUser;
    }

    ResponseVO<ImUserData> toIdUser = imUserDataService.getUserInfoByID(imFriendship.getToId(), imFriendship.getAppId());
    if (!toIdUser.isOk()) {
      return toIdUser;
    }
